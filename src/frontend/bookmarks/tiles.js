...
export const addBookmarkDates = (tiles, bookmarks) => {
  ...
  return tiles.map((tile) => {
    const bookmark = bookmarksByUrl.get(tile.url);
...
const fetchAll = async (bookmarks = []) => {
  const tileIds = await fetchIds();
  const tiles = await fetchFromStorage(tileIds);
  const kind = browser();

  const normalizedTiles = tileIds.map((id) => {
    return normalizeTile(id, tiles[id], kind);
  });

  return addBookmarkDates(normalizedTiles, bookmarks);
};
